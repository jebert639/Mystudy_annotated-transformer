...
> 本仓库以原版 [harvardnlp/annotated-transformer](https://github.com/harvardnlp/annotated-transformer)（MIT License）为基础，`transformer_model.py` 等核心实现及本页介绍的训练/推理工具均为中文注释的实践版本。
...
├── transformer_model.py          # ★核心 模型架构（EncoderDecoder/注意力/FFN/make_model）
├── train_utils.py                # ★核心 训练组件（Batch/标签平滑/LR调度/数据集/贪心解码）
├── train_runner.py               # ★核心 完整训练/推理流程（底层 train()/infer()）
├── plot_utils.py                 # 训练曲线绘制（loss / lr / 合并图）
│
├── transformer_api.py            # ★新增 一站式封装：train()/list_models()/infer() + CLI
├── model_gui.py                  # ★新增 Tkinter 推理界面
├── demo_train.py                 # ★新增 最小训练示例（冒烟测试）
├── launch_gui.bat                # ★新增 双击启动推理界面
├── TOOLKIT_README.md             # ★新增 高层封装详细文档
├── PROJECT_INTRO.md              # ★新增 项目详细介绍
│
├── train_sentence_completion.py  # 早期命令行入口（argparse，功能已被 transformer_api 覆盖）
├── check_data.py                 # 语料检查：段落数/数据划分/多小说占比统计
├── draw_dataflow.py              # 生成张量形状数据流图
├── text.py                       # CUDA 可用性小测试
│
├── the_annotated_transformer.py  # 原版注释论文源码（jupytext percent 格式）
├── AnnotatedTransformer.ipynb    # 原版注释论文 notebook（英文）
├── train_utils.ipynb             # ★新增 train_utils.py 中文循序渐进学习笔记
...
│
├── the-verdict.txt               # 默认训练语料（英文短篇，约 2 万字符）
...
（标 ★ 的分两组：`★核心` = 模型与训练全部逻辑；`★新增` = 在不改核心代码前提下添加的封装/界面/文档。）

### 核心文件详解

| 文件 | 内容 |
|---|---|
| `transformer_model.py` | 按论文架构图顺序组织：`EncoderDecoder` 总架构 → `Embeddings` 嵌入（含 √d_model 缩放）→ `PositionalEncoding` 正弦位置编码 → `attention` 缩放点积注意力 → `MultiHeadedAttention` 多头注意力 → `LayerNorm`/`SublayerConnection` 残差连接（Pre-LN）→ `PositionwiseFeedForward` FFN → 编码器/解码器堆叠 → `Generator` + `make_model()`（同词表时嵌入与输出投影权重共享、Xavier 初始化）。每块都附论文原文位置与逐行中文注释 |
| `train_utils.py` | `Batch`（src/tgt 掩码构造）、`LabelSmoothing` 标签平滑（KL 散度）、`SimpleLossCompute`、Noam 学习率 `rate()`（warmup 公式）、`run_epoch()` 训练循环、`CharTokenizer` 字符级分词器、`BPETokenizer`（tiktoken gpt2 + 3 特殊 token）、`TextDataset` 滑动窗口数据集（块前半作 prompt 进编码器、后半作续写目标）、`collate_batch` 批处理、`greedy_decode`（支持温度采样，规避 log_softmax 双重缩放陷阱）、`complete_sentence` 句子补全 |
| `train_runner.py` | 底层 `train()`：读数据 → 段落级 8:2 划分 → 建模 → DataLoader → Adam+Noam warmup 训练 → 每轮验证并按验证 loss 保存 `best_model.pt` → 保存 `final_model.pt` 并出三张曲线图；`infer()`：按 checkpoint 里的 config 重建模型并补全 |
| `plot_utils.py` | `plot_loss_curve` / `plot_lr_curve` / `plot_loss_lr_combined` 三张图，全部保存到输出目录 |

---

## 使用指南

以下命令均在 `conda activate annotated-transformer` 后、项目根目录执行。
...
python transformer_api.py train d_model=256 seq_len=48 data_path=combined.txt
```
...
train()                                                       # 默认配置训练 the-verdict.txt
train(preset="tiny")                                          # 冒烟测试预设
train(preset="fast", d_model=384, num_epochs=50)              # 预设基础上覆盖任意参数
train(data_path="combined.txt", output_dir="exp_A", seed=0)   # 换语料/输出目录/随机种子
```

- 未指定的参数沿用 `train_runner.train()` 的默认值；预设与显式参数可任意组合，显式参数优先。
- `output_dir` 已存在时自动递增（`output` → `output_2` → …），传 `overwrite=True` 强制覆盖。
...
自动扫描所有 `output*/` 目录下的 `.pt`，打印每个模型的类型（best/final）、epoch、val_loss、结构配置、大小、修改时间。Python 中用 `list_models()` 可拿到结构化列表。

### 3. 推理

```bash
# 命令行: 不写 --model 时自动选 val_loss 最低的 best 模型
python transformer_api.py infer "I had always thought" \
    --model output/best_model.pt --max_len 100 --temperature 0.7
```
...
pack = load_model("output/best_model.pt")         # 反复生成时加载一次
...
```bash
python model_gui.py           # 命令行启动
python transformer_api.py gui # 或经工具箱启动
```

功能：左侧自动列出所有模型（类型/Epoch/Val Loss/结构/大小），双击加载（后台线程，界面不卡）；右侧显示模型详情（参数量/设备/训练进度），输入前缀 → 调温度与最大长度 → 「生成补全」；结果区保留历史（含模型名/温度/耗时）；「查看训练曲线」调用系统看图工具打开该模型的 `loss_curve.png`；「浏览 .pt 文件」可加载任意位置的 checkpoint。

### 5. 早期入口与辅助脚本

```bash
python train_sentence_completion.py --infer "I had always thought" --model output/best_model.pt
                                    # 早期入口, 保留兼容; 不带 --infer 即进入训练
python check_data.py combined.txt   # 语料段落统计 / 模拟数据划分
python draw_dataflow.py             # 重新生成 transformer_dataflow.png
python text.py                      # 检查 CUDA 是否可用
```
...
每个输出目录（`output`、`output_2`、…）包含：

| 文件 | 说明 |
|---|---|
| `best_model.pt` | 按验证 loss 保存的最佳模型，内含 `model_state_dict`、`config`、`epoch`、`best_val_loss` |
...
调用 `infer()` 时自动加载 `best_model.pt`（若未指定 `model_path`）；`list_models()` 扫描所有 `output*/` 目录。
...
| `transformer_dataflow.md` / `.png` | 张量形状数据流：`seq_len=64` 的块如何切成 `src=(B,32)` 与 `tgt=(B,33)`，逐层形状变化（含最容易看错的形状细节提醒） |
...
以下问题均出现在 Windows + conda 环境中，`transformer_api.py` / `launch_gui.bat` 已在代码层规避，直接使用即可；若自行改动启动方式请留意：
...
- 中文注释 & 新增工具：Copyright (c) 2024 [Your Name]
